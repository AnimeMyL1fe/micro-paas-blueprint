"""Генерация Terraform tfvars и Ansible host_vars."""
import argparse
import json
...
with open(f"blueprints/{args.template}/blueprint.yml", encoding="utf-8") as f:
    inbound_data = yaml.safe_load(f)

with open(f"blueprints/{args.template}/profiles/{args.preset}.yml", encoding="utf-8") as f:
...
def get_values(f_file, find_text):
    """Вернуть список значений указанного поля из instances."""
    array = []
...
def get_pool(pool, exists_array):
    """Вернуть и зарезервировать первое неиспользованное значение из пула."""
    for value in pool:
